...
<tr><th>Patient ID</th><th>Encounter ID</th><th>Doctor</th><th>Nurse</th><th>Status</th><th>Action</th></tr>
<c:forEach items="${list}" var="req">
<tr><td>${req.patientid}<c:set var="vswrid" value="${req.workrequestid }"></c:set><c:set var="pid" value="${req.patientid}"></c:set></td><td>${req.encounterid}<c:set var="eid" value="${req.encounterid}" ></c:set></td><td>${req.doctorname }</td><td>${req.nursename}</td><td>${req.workrequeststatus}</td><td><a href="/webtools/doctor/viewvs?pid=${pid}&eid=${eid}&vswrid=${vswrid}" class="btn btn-info">View VitalSigns</a></td></tr>
</c:forEach>
